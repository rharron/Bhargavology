...
class PairOfTernaryQuadraticForms(SageObject):
    ...
    def maximal_pair(self, N=None):
        if N is None:
            raise NotImplementedError('N cannot be None yet.')
        ps = []
        for p, e in N.factor():
            ps += [p] * e
        #Ls = self.lambdas()
        ...
        while len(ps) > 0:
            ...
            if case3:
                print "It's case 3"
                raise NotImplementedError('Have not done case 3 yet.')
            for i, j, k in Permutations([1, 2, 3]):
                cases = []
                if AB.lambda_ijkl([i, i, i, j]) % p^2 != 0:
                    continue
                if AB.lambda_ijkl([i, i, i, k]) % p^2 == 0:
                    cases.append(1)
                if AB.lambda_ijkl([i, i, j, j]) % p^2 == 0 and AB.lambda_ijkl([i, j, j, j]) % p^2 == 0:
                    cases.append(2)
                case1 = False
                case2 = False
                while len(cases) > 0:
                    case = cases.pop()
                    if case == 1:
                        if AB.lambda_ijkl([i, i, j, j]) % p == 0 and AB.lambda_ijkl([i, i, j, k]) % p == 0 and AB.lambda_ijkl([i, i, k, k]) % p == 0 and AB.lambda_ijkl([i, j, i, k]) % p == 0:
                            case1 = True
                            break
                    elif case == 2:
                        print "Maybe case 2?"
                ...
                if case1:
                    ...
                    #Bs = B.row(i+1)
                    F = GF(p)
                    a = [F(AB._as[ij_to_cs[(i,i)]]/p), F(AB._as[ij_to_cs[(i,j)]]), F(AB._as[ij_to_cs[(i,k)]])]
                    b = [F(AB._bs[ij_to_cs[(i,i)]]/p), F(AB._bs[ij_to_cs[(i,j)]]), F(AB._bs[ij_to_cs[(i,k)]])]
                    u, v = Matrix([a, b]).left_kernel().basis()[0]
                    u = u.lift()
                    v = v.lift()
                    g = Matrix(2, [1, 0, u, v])
                    M = identity_matrix(QQ, 3)
                    M[i-1,i-1] = 1/p
                    AB = AB.act_by_GL2(g).act_by_GL2(Matrix(2, [p, 0, 0, 1])).act_by_GL3(M)
                    show(g)
                    show(M)
                    break
                if case2:
                    print "It's case 2"
                    raise NotImplementedError('Cannot do case 2 yet.')
            if not case1 and not case2:
                A,B = AB.matrix_pair()
                print A
                print
                print B
                print AB.disc().factor()
